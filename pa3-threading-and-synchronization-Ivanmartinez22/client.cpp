...
void worker_thread_function (FIFORequestChannel* chan, int buffercapacity, BoundedBuffer &request_buffer, BoundedBuffer &reponse_buffer) {
    // functionality of the worker threads
    for(;;){
...
int main (int argc, char* argv[]) {
    ...
    for (int i = 0; i < w; i++) {
		...
		FIFORequestChannel * chan_loc = new FIFORequestChannel(loc_name, FIFORequestChannel::CLIENT_SIDE); // channel switched to new channel
    ...
    for (int i = 0; i < w; i++) {
        FIFORequestChannel * in = channels[i];
