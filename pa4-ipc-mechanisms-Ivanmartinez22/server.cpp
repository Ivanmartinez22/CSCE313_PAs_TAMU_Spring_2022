...
void process_newchannel_request (RequestChannel* _channel) {
	...
	// RequestChannel* data_channel = new FIFORequestChannel(new_channel_name, RequestChannel::SERVER_SIDE); // if else statement !!!!!!!!
	RequestChannel* data_channel; // if else statement !!!!!!!!
	if(request_type == "f"){
        data_channel = new FIFORequestChannel(new_channel_name, RequestChannel::SERVER_SIDE); //effectively connecting to server
    }
    else if(request_type == "q"){
        data_channel = new MQRequestChannel(new_channel_name, RequestChannel::SERVER_SIDE, buffercapacity); //effectively connecting to server
		//data_channel = new FIFORequestChannel(new_channel_name, RequestChannel::SERVER_SIDE); //effectively connecting to server
    }
    else{
        //SHM case 
		data_channel = new SHMRequestChannel(new_channel_name, RequestChannel::SERVER_SIDE, buffercapacity); //effectively connecting to server
    }
...
void process_file_request (RequestChannel* rc, char* request) {
	...
	if (f.length > buffercapacity) {
		cerr << "Client is requesting a chunk bigger than server's capacity: " << rc->name() << endl;
...
void process_request (RequestChannel* rc, char* _request) {
...
void handle_process_loop (RequestChannel* channel) {
	...
	while (true) {
		// cout << "server handle loop" << endl;
		int nbytes = channel->cread(buffer, buffercapacity);
		// cout << "read from channel" << endl;
		if (nbytes < 0) {
		...
		// cout << "processing other request on server" << endl;
...
int main (int argc, char* argv[]) {
	...
	RequestChannel* control_channel; //if else statement
	if(request_type == "f"){
        control_channel = new FIFORequestChannel("control", RequestChannel::SERVER_SIDE); //effectively connecting to server
    }
    else if(request_type == "q"){
        control_channel = new MQRequestChannel("control", RequestChannel::SERVER_SIDE, buffercapacity); //effectively connecting to server
		//control_channel = new FIFORequestChannel("control", RequestChannel::SERVER_SIDE); //effectively connecting to server
    }
    else{
        //SHM case 
		control_channel = new SHMRequestChannel("control", RequestChannel::SERVER_SIDE, buffercapacity); //effectively connecting to server
    }
